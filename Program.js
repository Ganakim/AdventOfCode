let input = String.raw`"\xa8br\x8bjr\""
"nq"
...
input.split('\n').forEach((line, i)=>{
  let unraw = line.slice(1, -1)
    .replace(/\\x(?<hex>\d\d)/g, (match, hex)=>String.fromCharCode(parseInt(hex, 16)))
    .replace(/\\\\/g, '\\')
    .replace(/\\"/g, '"')
  console.log(`${line.length}`.padStart(2) + ',' + `${unraw.length}`.padStart(2), '|', line, '|', unraw)
  total += line.length - unraw.length
})
